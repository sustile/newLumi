...
class EmojiKeyboard {
      ...
      this.stickyObserver = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            ...
            const scrollPos =
              entry.target.offsetParent.scrollTop - entry.target.offsetTop;
            if (scrollPos >= 1) {
  ...
  close() {
    ...
    kb.classList.add("emojikb-hidden");
  }

  toggle_window() {
    let kb = document.getElementById("emojikb-maindiv");
    if (!kb) {
      if (this.auto_reconstruction) kb = this.get_keyboard(document);
      else return;
    }
  ...
  get_keyboard(document) {
    let kb = document.getElementById("emojikb-maindiv");
    if (!kb) kb = this.create_keyboard(document);
    return kb;
  }
  ...
  hover_out_emoji(kb, event) {
    const document = event.target.ownerDocument;
    document.getElementById("emojikb-textinput").placeholder =
      kb.default_placeholder;
  ...
  click_on_category(kb, event, stay) {
    ...
    if (!stay) {
      ...
      const targets = target.ownerDocument.querySelectorAll(
        'div.emojikb-categname[data-emojikb_categ="' + categ + '"]'
      );
      if (!targets[0]) return;
  ...
  on_typing(event) {
    if (event.target.value === "") {
      event.target.ownerDocument
        .querySelectorAll("img.emojikb-emoji")
        .forEach((e) => {
          e.classList.toggle("emojikb-hidden", false);
        });
      event.target.ownerDocument
        .querySelectorAll("div.emojikb-categ")
        .forEach((e) => {
          e.classList.toggle("emojikb-hidden", false);
        });
      return;
    ...
    event.target.ownerDocument
      ...
      .forEach((e) => {
        ...
        e.classList.toggle(
          "emojikb-hidden",
          e.childElementCount <= hidden_nbr + 1
        );
      });
  }
      ...
      .then(async (response) => {
        ...
        if (
          ...
          json.emojis.forEach((emoji) => {
            if (
              emoji.shortname.includes("_skin_tone") ||
              emoji.unicode.includes(":")
            )
              // invalid emojis
              return;
            ...
            this.emojis.get(categ).push({
              ...
              url:
                ...
                emoji.unicode.replace(/ /g, "-").toLowerCase() +
                ".png",
          ...
          console.error(
            "Oops, can't get the usual emojis list! (invalid format)"
          );
        }
      })
      .catch((err) => {});
  ...
  find_right_category(subcategory) {
    ...
    try {
      ...
      for (const [k, v] of categories) {
        if (v.includes(sub_name)) return k;
      }
    } catch (err) {
      console.error(err);
    }
  ...
  create_keyboard(document) {
    ...
    const parse_svg = (x) =>
      dom_parser.parseFromString(x, "text/xml").firstChild;
    ...
    for (const v of [
      ...
      for (const emoji of this.emojis
        ...
        img.addEventListener("error", (err) => {
          ...
          const index = this.emojis
            .get(data.category)
            .findIndex((e) => e.name == data.name);
